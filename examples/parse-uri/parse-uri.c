...
static struct uri_struct parse_uri(strview_t uri)
{
	...
	if(strview_is_valid(strview_find_first(uri, "://")))
	{
		result.scheme = strview_split_first_delim(&uri, ":",false);
		uri = strview_sub(uri, 2, INT_MAX);
	};

	result.user = strview_split_first_delim(&uri, "@",false);
	if(!strview_is_valid(uri))
		strview_swap(&result.user, &uri);

	if(!strview_is_valid(strview_find_first(uri, ":")))
	{
		tmp = strview_split_first_delim(&uri, "/",false);
		if(strview_is_valid(uri))
		{
			result.path = uri;
			result.host = tmp;
